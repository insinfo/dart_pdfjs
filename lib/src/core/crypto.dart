...
class ARCFourCipher {
  ...
  Uint8List decryptBlock(Uint8List data,
      [bool finalize = false, Uint8List? iv]) {
...
abstract class AESBaseCipher {
  ...
  Uint8List decryptBlock(Uint8List data,
      [bool finalize = false, Uint8List? ivArg]) {
    final sourceLength = data.length;
    ...
    if (ivArg != null) {
      ...
      for (int i = 0;
          currentBufferLength < 16 && i < sourceLength;
          ++i, ++currentBufferLength) {
        currentBuffer[currentBufferLength] = data[i];
      }
      if (currentBufferLength < 16) {
        this.bufferPosition = currentBufferLength;
...
abstract class PDFBase {
  ...
  bool checkOwnerPassword(Uint8List password, Uint8List ownerValidationSalt,
      Uint8List userBytes, Uint8List ownerPassword) {
    final hashData = Uint8List(password.length + 56);
    hashData.setRange(0, password.length, password);
    hashData.setRange(password.length,
        password.length + ownerValidationSalt.length, ownerValidationSalt);
    hashData.setRange(password.length + ownerValidationSalt.length,
        hashData.length, userBytes);
    final result = _hash(password, hashData, userBytes);
    return isArrayEqual(result, ownerPassword);
  }

  bool checkUserPassword(Uint8List password, Uint8List userValidationSalt,
  ...
  Uint8List getOwnerKey(Uint8List password, Uint8List ownerKeySalt,
      Uint8List userBytes, Uint8List ownerEncryption) {
    final hashData = Uint8List(password.length + 56);
    hashData.setRange(0, password.length, password);
    hashData.setRange(
        password.length, password.length + ownerKeySalt.length, ownerKeySalt);
    hashData.setRange(
        password.length + ownerKeySalt.length, hashData.length, userBytes);
  ...
  Uint8List getUserKey(
      Uint8List password, Uint8List userKeySalt, Uint8List userEncryption) {
    final hashData = Uint8List(password.length + 8);
    hashData.setRange(0, password.length, password);
...
class PDF20 extends PDFBase {
  ...
  Uint8List _hash(Uint8List password, Uint8List input, Uint8List userBytes) {
    ...
    while (i < 64 || (e.isNotEmpty && e.last > i - 32)) {
      ...
      int writeOffset = 0;
      combinedArray.setRange(
          writeOffset, writeOffset + password.length, password);
      writeOffset += password.length;
      combinedArray.setRange(writeOffset, writeOffset + k.length, k);
      writeOffset += k.length;
      combinedArray.setRange(
          writeOffset, writeOffset + userBytes.length, userBytes);
