...
class JpxError extends BaseException {
  JpxError(String message) : super('JPX error: $message', 'JpxError');
}
...
class _StreamAdapter implements JpxByteStream {
...
class JpxImage {
  ...
  static bool _useWasm = false;
  static bool _useWorkerFetch = true;
  static String? _wasmUrl;

  static bool get useWasm => _useWasm;
  static bool get useWorkerFetch => _useWorkerFetch;
  static String? get wasmUrl => _wasmUrl;
  static dynamic get handler => _handler;
  ...
  /// Decodes JPEG 2000 image bytes returning the raw RGBA pixels.
  static Future<Uint8List> decode(
    Uint8List data, [
    dynamic decoderOptions,
  ]) async {
    final jpx = JpxImage();
    jpx.parse(data);
    if (jpx.tiles.isEmpty) {
      throw JpxError('No tiles found in decoded image');
    }
    if (jpx.tiles.length == 1) {
      return jpx.tiles[0].items;
    }
    // Multiple tiles: stitch into full image buffer.
    final totalBytes = jpx.width * jpx.height * 4;
    final fullImage = Uint8List(totalBytes);
    for (final tile in jpx.tiles) {
      final tileStride = tile.width * 4;
      final fullStride = jpx.width * 4;
      for (var row = 0; row < tile.height; row++) {
        final srcOffset = row * tileStride;
        final dstOffset = (tile.top + row) * fullStride + (tile.left * 4);
        fullImage.setRange(
          dstOffset,
          dstOffset + tileStride,
          tile.items,
          srcOffset,
        );
      }
  ...
  /// Parses basic image properties from a stream or byte list.
  static JpxImageProperties parseImageProperties(dynamic stream) {
    if (stream is Uint8List || stream is ByteBuffer) {
      stream = Stream(stream);
    }
    JpxByteStream byteStream;
    if (stream is JpxByteStream) {
      byteStream = stream;
    } else if (stream is BaseStream) {
      byteStream = _StreamAdapter(stream);
    } else {
      throw JpxError('Invalid data format, must be a stream or typed data.');
    }

    var newByte = byteStream.getByte();
    while (newByte >= 0) {
      final oldByte = newByte;
      newByte = byteStream.getByte();
      final code = (oldByte << 8) | newByte;
      if (code == 0xff51) {
        byteStream.skip(4);
        final xsiz = byteStream.getInt32() & 0xffffffff;
        final ysiz = byteStream.getInt32() & 0xffffffff;
        final xOsiz = byteStream.getInt32() & 0xffffffff;
        final yOsiz = byteStream.getInt32() & 0xffffffff;
        byteStream.skip(16);
        final csiz = byteStream.getUint16();
  ...
  void _buildPackets(Map<String, dynamic> context) {
    ...
    for (var c = 0; c < componentsCount; c++) {
      ...
      for (var r = 0; r <= decompositionLevelsCount; r++) {
        ...
        resolution['try1'] = (_i(component['tcx1']) / scale).ceil();
...
class _BitModel {
  ...
  static final Uint8List _llAndLhContextLabel = Uint8List.fromList(<int>[
    0, 5, 8, 0, 3, 7, 8, 0, 4, 7, 8, 0, 0, 0, 0, 0,
    1, 6, 8, 0, 3, 7, 8, 0, 4, 7, 8, 0, 0, 0, 0, 0,
    2, 6, 8, 0, 3, 7, 8, 0, 4, 7, 8, 0, 0, 0, 0, 0,
    2, 6, 8, 0, 3, 7, 8, 0, 4, 7, 8, 0, 0, 0, 0, 0,
    2, 6, 8, 0, 3, 7, 8, 0, 4, 7, 8,
  ]);
  static final Uint8List _hlContextLabel = Uint8List.fromList(<int>[
    0, 3, 4, 0, 5, 7, 7, 0, 8, 8, 8, 0, 0, 0, 0, 0,
    1, 3, 4, 0, 6, 7, 7, 0, 8, 8, 8, 0, 0, 0, 0, 0,
    2, 3, 4, 0, 6, 7, 7, 0, 8, 8, 8, 0, 0, 0, 0, 0,
    2, 3, 4, 0, 6, 7, 7, 0, 8, 8, 8, 0, 0, 0, 0, 0,
    2, 3, 4, 0, 6, 7, 7, 0, 8, 8, 8,
  ]);
  static final Uint8List _hhContextLabel = Uint8List.fromList(<int>[
    0, 1, 2, 0, 1, 2, 2, 0, 2, 2, 2, 0, 0, 0, 0, 0,
    3, 4, 5, 0, 4, 5, 5, 0, 5, 5, 5, 0, 0, 0, 0, 0,
    6, 7, 7, 0, 7, 7, 7, 0, 7, 7, 7, 0, 0, 0, 0, 0,
    8, 8, 8, 0, 8, 8, 8, 0, 8, 8, 8, 0, 0, 0, 0, 0,
