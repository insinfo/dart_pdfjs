...
final Map<int, int> _specialPUASymbols = () {
  final Map<int, int> t = {};
...
int mapSpecialUnicodeValues(int code) {
  if (code >= 0xfff0 && code <= 0xffff) {
    // Specials unicode block.
    return 0;
  } else if (code >= 0xf600 && code <= 0xf8ff) {
    return _specialPUASymbols[code] ?? code;
...
int getUnicodeForGlyph(String name, Map<String, int> glyphsUnicodeMap) {
  final unicode = glyphsUnicodeMap[name];
  if (unicode != null) {
    return unicode;
  }
  if (name.isEmpty) {
...
const List<List<int>> unicodeRanges = [
...
int getUnicodeRangeFor(int value, [int lastPosition = -1]) {
  if (lastPosition != -1) {
    final range = unicodeRanges[lastPosition];
    for (int i = 0, ii = range.length; i < ii; i += 2) {
      if (value >= range[i] && value <= range[i + 1]) {
        return lastPosition;
      }
    }
  }
  for (int i = 0, ii = unicodeRanges.length; i < ii; i++) {
    final range = unicodeRanges[i];
    for (int j = 0, jj = range.length; j < jj; j += 2) {
...
class UnicodeCategory {
  ...
  final bool isZeroWidthDiacritic;
  final bool isInvisibleFormatMark;

  UnicodeCategory({
    required this.isWhitespace,
    required this.isZeroWidthDiacritic,
    required this.isInvisibleFormatMark,
  });
}

final RegExp _specialCharRegExp = RegExp(r'^(\s)|(\p{Mn})|(\p{Cf})$', unicode: true);
final Map<String, UnicodeCategory> _categoryCache = {};

UnicodeCategory getCharUnicodeCategory(String char) {
  final cachedCategory = _categoryCache[char];
  if (cachedCategory != null) {
    return cachedCategory;
  }
  final match = _specialCharRegExp.firstMatch(char);
  final category = UnicodeCategory(
    isWhitespace: match?.group(1) != null,
    isZeroWidthDiacritic: match?.group(2) != null,
    isInvisibleFormatMark: match?.group(3) != null,
  );
  _categoryCache[char] = category;
  return category;
}

void clearUnicodeCaches() {
